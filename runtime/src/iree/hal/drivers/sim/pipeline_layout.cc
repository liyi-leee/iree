...
//===----------------------------------------------------------------------===//
// iree_hal_sim_descriptor_set_layout_t
//===----------------------------------------------------------------------===//

typedef struct iree_hal_sim_descriptor_set_layout_t {
...
extern const iree_hal_descriptor_set_layout_vtable_t
    iree_hal_sim_descriptor_set_layout_vtable;
}  // namespace

static iree_hal_sim_descriptor_set_layout_t*
iree_hal_sim_descriptor_set_layout_cast(
    iree_hal_descriptor_set_layout_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_sim_descriptor_set_layout_vtable);
  return (iree_hal_sim_descriptor_set_layout_t*)base_value;
}

static const iree_hal_sim_descriptor_set_layout_t*
iree_hal_sim_descriptor_set_layout_const_cast(
    const iree_hal_descriptor_set_layout_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_sim_descriptor_set_layout_vtable);
  return (const iree_hal_sim_descriptor_set_layout_t*)base_value;
}

iree_status_t iree_hal_sim_descriptor_set_layout_create(
  ...
  *out_descriptor_set_layout = NULL;
  ...
  iree_hal_sim_descriptor_set_layout_t* descriptor_set_layout = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*descriptor_set_layout),
                                (void**)&descriptor_set_layout));

  iree_hal_resource_initialize(&iree_hal_sim_descriptor_set_layout_vtable,
...
iree_host_size_t iree_hal_sim_descriptor_set_layout_binding_count(
    const iree_hal_descriptor_set_layout_t* base_descriptor_set_layout) {
  const iree_hal_sim_descriptor_set_layout_t* descriptor_set_layout =
      iree_hal_sim_descriptor_set_layout_const_cast(base_descriptor_set_layout);
  return descriptor_set_layout->binding_count;
}

static void iree_hal_sim_descriptor_set_layout_destroy(
    iree_hal_descriptor_set_layout_t* base_descriptor_set_layout) {
  iree_hal_sim_descriptor_set_layout_t* descriptor_set_layout =
      iree_hal_sim_descriptor_set_layout_cast(base_descriptor_set_layout);
  IREE_TRACE_ZONE_BEGIN(z0);
...
const iree_hal_descriptor_set_layout_vtable_t
    iree_hal_sim_descriptor_set_layout_vtable = {
        /*.destroy = */ iree_hal_sim_descriptor_set_layout_destroy,
};
}  // namespace

//===----------------------------------------------------------------------===//
// iree_hal_sim_pipeline_layout_t
//===------------------------------------------------------ ---------------===//

typedef struct iree_hal_sim_pipeline_layout_t {
  // Abstract resource used for injecting reference counting and vtable; must be
  // at offset 0.
  iree_hal_resource_t resource;
...
extern const iree_hal_pipeline_layout_vtable_t
    iree_hal_sim_pipeline_layout_vtable;
}  // namespace

static iree_hal_sim_pipeline_layout_t* iree_hal_sim_pipeline_layout_cast(
    iree_hal_pipeline_layout_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_sim_pipeline_layout_vtable);
  return (iree_hal_sim_pipeline_layout_t*)base_value;
}

static const iree_hal_sim_pipeline_layout_t*
iree_hal_sim_pipeline_layout_const_cast(
    const iree_hal_pipeline_layout_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_sim_pipeline_layout_vtable);
  return (const iree_hal_sim_pipeline_layout_t*)base_value;
}

iree_status_t iree_hal_sim_pipeline_layout_create(
    iree_host_size_t set_layout_count,
  ...
  *out_pipeline_layout = NULL;
  ...
  iree_hal_sim_pipeline_layout_t* pipeline_layout = NULL;
  ...
  iree_hal_resource_initialize(&iree_hal_sim_pipeline_layout_vtable,
  ...
  for (iree_host_size_t i = 0; i < set_layout_count; ++i) {
    ...
    base_index +=
        iree_hal_sim_descriptor_set_layout_binding_count(set_layouts[i]);
  }
...
static void iree_hal_sim_pipeline_layout_destroy(
    iree_hal_pipeline_layout_t* base_pipeline_layout) {
  iree_hal_sim_pipeline_layout_t* pipeline_layout =
      iree_hal_sim_pipeline_layout_cast(base_pipeline_layout);
  IREE_TRACE_ZONE_BEGIN(z0);
...
iree_host_size_t iree_hal_sim_pipeline_layout_descriptor_set_count(
    const iree_hal_pipeline_layout_t* base_pipeline_layout) {
  const iree_hal_sim_pipeline_layout_t* pipeline_layout =
      iree_hal_sim_pipeline_layout_const_cast(base_pipeline_layout);
...
iree_host_size_t iree_hal_sim_pipeline_layout_base_binding_index(
...
namespace {
const iree_hal_pipeline_layout_vtable_t iree_hal_sim_pipeline_layout_vtable = {
    /*.destroy = */ iree_hal_sim_pipeline_layout_destroy,
};
}  // namespace
