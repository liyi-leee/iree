...
#include "iree/schemas/sim_executable_def_reader.h"
#include "iree/schemas/sim_executable_def_verifier.h"
#include "iree/base/api.h"
#include "iree/base/internal/flatcc/parsing.h"

typedef struct iree_hal_sim_native_executable_t {
  ...
  iree_hal_sim_kernel_params_t entry_points[];
} iree_hal_sim_native_executable_t;

namespace {
extern const iree_hal_executable_vtable_t iree_hal_sim_native_executable_vtable;
}  // namespace

static iree_hal_sim_native_executable_t* iree_hal_sim_native_executable_cast(
    iree_hal_executable_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_sim_native_executable_vtable);
  return (iree_hal_sim_native_executable_t*)base_value;
}
...
static iree_status_t iree_hal_sim_native_executable_flatbuffer_verify(
  ...
  int verify_ret = iree_hal_sim_ExecutableDef_verify_as_root(
  ...
  iree_hal_sim_ExecutableDef_table_t executable_def =
      iree_hal_sim_ExecutableDef_as_root(flatbuffer_data.data);

  flatbuffers_string_vec_t entry_points_vec =
      iree_hal_sim_ExecutableDef_entry_points_get(executable_def);
  ...
  flatbuffers_string_t xclbin =
      iree_hal_sim_ExecutableDef_xclbin_get(executable_def);
  if (flatbuffers_string_len(xclbin) == 0) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT, "no xclbin present");
  }

  iree_hal_sim_AsmInstDef_vec_t asm_instr =
      iree_hal_sim_ExecutableDef_asm_instrs_get(executable_def);
  size_t number_asm_instr = iree_hal_sim_AsmInstDef_vec_len(asm_instr);
...
iree_status_t iree_hal_sim_native_executable_create(
    sim::device device, const iree_hal_executable_params_t* executable_params,
  ...
  *out_executable = NULL;
  iree_hal_sim_native_executable_t* executable = NULL;

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_sim_native_executable_flatbuffer_verify(
              executable_params->executable_data));

  iree_hal_sim_ExecutableDef_table_t executable_def =
      iree_hal_sim_ExecutableDef_as_root(
  ...
  //SIM API needs this vector and cant actually read a void*.
  std::vector<char> xclbinVector(xclbin_fb,
                                 xclbin_fb + flatbuffers_string_len(xclbin_fb));

  iree_hal_sim_AsmInstDef_vec_t asm_instrs_vec =
      iree_hal_sim_ExecutableDef_asm_instrs_get(executable_def);
  ...
  iree_hal_resource_initialize(&iree_hal_sim_native_executable_vtable,
                               &executable->resource);
  // sim::xclbin xclbin;
  // try {
  //   xclbin = sim::xclbin(xclbinVector);
  // } catch (std::runtime_error& e) {
  //   return iree_make_status(IREE_STATUS_INTERNAL, "XCLBIN load error: %s",
  //                           e.what());
  // }
  // device.register_xclbin(xclbin);
  // sim::hw_context context(device, xclbin.get_uuid());
  executable->host_allocator = host_allocator;
  executable->entry_point_count = entry_point_count;
  for (iree_host_size_t i = 0; i < entry_point_count; i++) {
    const char* entry_name = flatbuffers_string_vec_at(entry_points_vec, i);
    iree_hal_sim_AsmInstDef_table_t asminst_def =
        iree_hal_sim_AsmInstDef_vec_at(asm_instrs_vec, i);
    flatbuffers_uint32_vec_t asm_inst =
        iree_hal_sim_AsmInstDef_asm_inst_get(asminst_def);
    uint32_t num_instr = flatbuffers_uint32_vec_len(asm_inst);
    std::unique_ptr<sim::kernel> kernel;
    std::unique_ptr<sim::bo> instr;

    //kernel = std::make_unique<sim::kernel>(context, entry_name);
    kernel = std::make_unique<sim::kernel>(device, entry_name);
    ...
    instr = std::make_unique<sim::bo>(device, num_instr * sizeof(uint32_t),
                                      sim::bo::flags::cacheable,
                                      0);
    ...
    instr->sync(SIM_BO_SYNC_BO_TO_DEVICE);
    iree_hal_sim_kernel_params_t* params = &executable->entry_points[i];
    ...
    IREE_TRACE({
      if (iree_hal_sim_ExecutableDef_source_locations_is_present(
              executable_def)) {
        iree_hal_sim_FileLineLocDef_vec_t source_locs_vec =
            iree_hal_sim_ExecutableDef_source_locations_get(
                executable_def);
        iree_hal_sim_FileLineLocDef_table_t source_loc =
            iree_hal_sim_FileLineLocDef_vec_at(source_locs_vec, i);
        flatbuffers_string_t filename =
            iree_hal_sim_FileLineLocDef_filename_get(source_loc);
        uint32_t line =
            iree_hal_sim_FileLineLocDef_line_get(source_loc);
...
static void iree_hal_sim_native_executable_destroy(
    iree_hal_executable_t* base_executable) {
  iree_hal_sim_native_executable_t* executable =
      iree_hal_sim_native_executable_cast(base_executable);
...
iree_status_t iree_hal_sim_native_executable_entry_point_kernel_params(
    iree_hal_executable_t* base_executable, int32_t entry_point,
    iree_hal_sim_kernel_params_t* out_params) {
  iree_hal_sim_native_executable_t* executable =
      iree_hal_sim_native_executable_cast(base_executable);
...
namespace {
const iree_hal_executable_vtable_t iree_hal_sim_native_executable_vtable = {
    /*.destroy=*/iree_hal_sim_native_executable_destroy,
};
}  // namespace
